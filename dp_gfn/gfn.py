import torch
from dp_gfn.utils.masking import encode, decode, base_mask
...
class StateBatch:
    ...
    def __init__(self, edges, labels, num_variables, device, root_first=True):
        ...
        self._data = {
            ...
            "mask": encode(base_mask(num_variables=num_variables, root_first=root_first, device=device).repeat(edges.shape[0], 1, 1)),
            "adjacency": encode(torch.zeros(edges.shape[0], num_variables, num_variables, device=device)),
            "isolated_root": torch.ones(edges.shape[0], dtype=torch.bool, device=device),
